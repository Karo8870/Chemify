...
export class Reaction {
  ...
  createIonFromOxide(oxide: string): { ion: string, valence: number } {
    ...
    if (parsedOxide[0].element === 'P') {
      return {
        ion: 'PO' + (Math.ceil(parsedOxide[1].amount / 2 + 1) > 1 ? Math.ceil(parsedOxide[1].amount / 2 + 1) : ''),
        valence: 3
      };
    }
    ...
    if (parsedOxide[0].amount === 2) {
      return {
        ion: parsedOxide[0].element + 'O' + (Math.ceil(parsedOxide[1].amount / 2) > 1 ? Math.ceil(parsedOxide[1].amount / 2) : ''),
        valence: this._parser.findElementBySymbol(parsedOxide[0].element).anionValence!
      };
    }

    return {
      ion: parsedOxide[0].element + 'O' + (parsedOxide[1].amount + 1),
      valence: this._parser.findElementBySymbol(parsedOxide[0].element).anionValence!
    };
  }
  ...
  combineAcidWithSalt(compounds: parsedCompoundModel[]): string[] {
    ...
    const v1 = compounds[0].parts[0].amount,
      v2 = anionValence! * compounds[1].parts[1].amount / compounds[1].parts[0].amount;
  ...
  combineOxideWithSalt(compounds: parsedCompoundModel[]): string[] {
    ...
    const v1 = 2 * compounds[0].parts[1].amount / compounds[0].parts[0].amount,
      v2 = anionValence * compounds[1].parts[1].amount / compounds[1].parts[0].amount;
  ...
  executeCommand(reaction: string): string[][] {
    return this.combine(reaction.replace(' ', '').split('+').map((el: any) => this._parser.parseString(el)));
  }
